...
for key in antenna_locations:
    ...
    for i in range(len(antenna_locations[key])):
        for j in range(i + 1, len(antenna_locations[key])):
            ...
            possible_antinodes.add((x1, y1))
            possible_antinodes.add((x2, y2))

            dx = x2 - x1
            dy = y2 - y1

            spot_1 = (x1 - dx, y1 - dy)
            while check_spot(*spot_1):
                possible_antinodes.add(spot_1)
                spot_1 = (spot_1[0] - dx, spot_1[1] - dy)
            spot_2 = (x2 + dx, y2 + dy)
            while check_spot(*spot_2):
                possible_antinodes.add(spot_2)
                spot_2 = (spot_2[0] + dx, spot_2[1] + dy)
