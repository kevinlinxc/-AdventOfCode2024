...
def recursive_check(target, operands, operator):
    if len(operands) == 2:
        if operator == "*":
            ...
            return int(str(operands[0]) + str(operands[1])) == target
    ...
    if operator == "*":
        ...
        result = int(str(first_two[0]) + str(first_two[1]))

    return (
        recursive_check(target, [result] + operands[2:], "*")
        or recursive_check(target, [result] + operands[2:], "+")
        or recursive_check(target, [result] + operands[2:], "||")
    )
...
def can_reach_target(target, operands):
    ...
    return (
        recursive_check(target, operands, "*")
        or recursive_check(target, operands, "+")
        or recursive_check(target, operands, "||")
    )
